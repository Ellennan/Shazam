...
class SQLDatabase(Database):
    ...
    3) get hashes with the highest number of collisions

        select hash, count(distinct song_id) as n
        from fingerprints
        group by `hash`
        order by n DESC;
    ...
    """ % (
        ...
        Database.FIELD_SONG_ID, Database.FIELD_SONG_ID,
    )
    ...
    INSERT_SONG = """
        INSERT INTO %s (%s, %s) values (%%s, UNHEX(%%s));
